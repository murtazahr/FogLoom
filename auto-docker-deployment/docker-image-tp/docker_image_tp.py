...
class DockerImageTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        try:
            ...
            logger.debug(
                f"Received image hash: {image_hash} for image: {image_name}, app_id: {app_id}, action: {action}")
            ...
            context.add_event(
                ...
                attributes=[("image_hash", image_hash), ("image_name", image_name), ("app_id", app_id),
                            ("action", action)]
            )
            logger.info(
                f"Emitted event for image: {image_name} with hash: {image_hash}, app_id: {app_id}, action: {action}")

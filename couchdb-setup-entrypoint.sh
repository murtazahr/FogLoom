...
setup_bidirectional_replication() {
    ...
    response=$(curl -s -X POST "http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${source}.local:${PORT}/_replicator" \
         -H "Content-Type: application/json" \
         -d "{
              \"_id\": \"${source}_to_${target}_${DB_NAME}\",
              \"source\": \"http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${source}.local:${PORT}/${DB_NAME}\",
              \"target\": \"http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${target}.local:${PORT}/${DB_NAME}\",
              \"continuous\": true
             }")
    log_message "Replication from ${source} to ${target} response: ${response}"

    # Setup replication from target to source
    response=$(curl -s -X POST "http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${target}.local:${PORT}/_replicator" \
         -H "Content-Type: application/json" \
         -d "{
              \"_id\": \"${target}_to_${source}_${DB_NAME}\",
              \"source\": \"http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${target}.local:${PORT}/${DB_NAME}\",
              \"target\": \"http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${source}.local:${PORT}/${DB_NAME}\",
              \"continuous\": true
             }")
...
for num in 0 1 2 3 4; do
    for db in _users _replicator; do
      response=$(curl -s -X PUT "http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@couchdb$num.local:5984/$db")
...
for node in couchdb0 couchdb1 couchdb2 couchdb3 couchdb4; do
    response=$(curl -s -X PUT "http://${COUCHDB_USER}:${COUCHDB_PASSWORD}@${node}.local:${PORT}/${DB_NAME}")
...
for i in 0 1 2 3; do
    for j in $(seq $((i+1)) 4); do
        setup_bidirectional_replication "couchdb$i" "couchdb$j"
    done
done
